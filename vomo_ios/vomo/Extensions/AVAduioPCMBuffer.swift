...
//         return Float(0)
         ...
         for i in 0..<ac_Lags_s.count {
             if ac_Lags_s[i] >= Float(1 / f0_range_max) {
                 lag_range_idx[0] = Float(i)
             } else if ac_Lags_s[i] > Float(1 / f0_range_min) {
                 lag_range_idx[1] = Float(i - 1)
             }
         }
 ...
 // CHANGED: extended AVAudioPCMBuffer to calculate intensity
  extension AVAudioPCMBuffer {
      ...
      public func getPitch(segmentSize segmentSize_samples: Int, startOffset startPositionOffset: Int, segments numOfSegments: Int, sampRate samplingRate: Float) -> [Float]? {
          ...
          var pitchValues = Array(repeating: Float(0), count: numOfSegments) // FIX: change anything named value
          var position = 0
          let chunkLength = segmentSize_samples
          let channelCount = Int(format.channelCount)
      ...
      private func getSegmentPitch(from block: [Float], segSize segmentSize: Int, sampRate samplingRate: Float) -> Float {
          ...
          // Initialize output pitch vector
          var f0_Hz = Array(repeating: 0, count: segmentSize)
          
          // Initialize vectors
          var ac_Sig = Array(repeating: 0, count: segmentSize)
          var ac_Lags_s = transpose(in_signal) / samplingRate // FIX: syntax, creating array with incremental numbers
          var ac_Scaling = linspace(1, ac_scalingMin, segmentSize) // FIX: syntax, replacement for linspace, decreasing array
          
          // Calculate auto-crrelation of window
          var padding = Array(repeating: 0, count: segmentSize-1)
          var padded_window = append(window) + append(padding) // FIX: syntax
          var ac_temp = vDSP.correlate(padded_window, withKernel: window)
          ...
          lag_range_idx[2] = find(ac_Lags_s > (1/f0_range[1]), 1) - 1 // FIX: syntax
              ...
              // Scale  signal's auto-correlation by window's auto-correlation, and scale result to add more towards smaller lags
              ac_Sig = ac_temp ./ awindow .* ac_Scaling // FIX: syntax, element-by-element division and multiply
          
              // Find peaks in search range, filter out peaks with magnitude smaller than peak threshold
              var peaks_mag: Array // Value of ac_Sig at that point
              var locs: Array // Location of peaks
              [peaks_mag, locs] = findpeaks( ac_Sig(lag_range_idx[1]:lag_range_idx[2]) ) // FIX: syntax, replace findpeaks fxn
              var peak_loc = find(peaks_mag > peakThreshold, 10) // FIX: syntax, replace find fxn, find first 10 peaks > threshold
          ...
          // To find the actual index of the peak in the in_signal, add offset of starting location of lag_range_index[1] to element index
              ...
              if isempty(peak_loc) { // FIX: syntax, replace isempty fxn
                  // If no peaks are above threshold, set f0 to 0 Hz
                  ac_f0_hz = 0 // FIX: syntax
              }
              else {
                  ...
                  for peak_index in 1..(peak_loc.count - 1) { // FIX: syntax
                      if ( peaks_mag(peak_loc[peak_index]) > peaks_mag(peak_loc[peak_index-1]) ) &&
                          ( peaks_mag(peak_loc[peak_index]) > peaks_mag(peak_loc[peak_index+1]) ) { // FIX: syntax of arrays/matrices
                          max_index = peak_index
                      }
                  } // End for
          
                  // Calculate f0, in Hz, using largest peak of auto-correlation
                  ac_f0_hz = Float(1) / ( ac_Lags_s( lag_range_idx[1] + locs(peak_loc[max_index]) ) ) // FIX: syntax of arrays/matrices
          ...
          // FIX: do I need private helper functions
         */
      } // End fxn, getSegmentPitch
  }
