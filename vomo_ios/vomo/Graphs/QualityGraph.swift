...
class QualityNodeModel: Identifiable, Codable {
    var data: Double
    var dataDate: Date
...
struct QualityGraph: View {
    ...
    @State private var points: [QualityNodeModel] = []
    @State private var firstPoint: QualityNodeModel = QualityNodeModel(data: 0.0, dataDate: .now, hasTreatment: false, afterDate: false, treatmentDate: .now, treatmentType: "")
                ...
                VStack(spacing: 0) {
                    ...
                    GeometryReader { geo in
                        VStack(spacing: 0) {
                            ...
                            Color.clear.frame(height: geo.size.height * nodes(cpp: point.data).1)
                        }
                    }
